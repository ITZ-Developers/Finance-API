...
public class DateUtils {
	...
	public static Date convertLocalDate2Date(LocalDate localDate){
		...
		return localDate == null ? null : Date.from(localDate.atStartOfDay(defaultZoneId).toInstant());
	}
	...
	// system is UTC, return date is UTC + 7
	public static Date convertUTCToVN(Date utcDate){
		Instant instant = utcDate.toInstant();
		ZonedDateTime zonedDateTime = ZonedDateTime.ofInstant(instant, ZoneOffset.UTC);
		ZonedDateTime updatedDateTime = zonedDateTime.plusHours(7);
		Instant updatedInstant = updatedDateTime.toInstant();
