...
import com.finance.dto.account.MyKeyDto;
...
public class AccountController extends ABasicController{
    ...
    @PostMapping(value = "/verify-key", produces = MediaType.APPLICATION_JSON_VALUE)
    public ApiMessageDto<String> verifyKey(@Valid @RequestBody VerifyKeyForm verifyKeyForm, BindingResult bindingResult){
        String decryptFinanceSecretKey= RSAUtils.decrypt(verifyKeyForm.getPrivateKey(), financeSecretKey);
        String decryptKeyInformationSecretKey = RSAUtils.decrypt(verifyKeyForm.getPrivateKey(), keyInformationSecretKey);
        String decryptPasswordSecretKey = RSAUtils.decrypt(verifyKeyForm.getPrivateKey(), encryptPasswordSecretKey);
        ...
        concurrentMap.put(FinanceConstant.PRIVATE_KEY, verifyKeyForm.getPrivateKey());
